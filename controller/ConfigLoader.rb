...
class ConfigLoader
    ...
    registerValue.each do |name, value|
      ...
      if attribute.has_key?('Array')
        ...
        unless value.is_a?(Integer)
          puts "Error: #{name} must be Integer"
          raise ""
        end
  ...
  def validateSelectableLogic
    ...
    if selectableLogic['HitNum Threshold'].class == Integer
  ...
  def validateTriggerValues
    ...
    unless mode.class == Integer && mode.to_i >= 0 && mode.to_i <= 7
